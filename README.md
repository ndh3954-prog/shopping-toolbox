...
- 헤더의 **데이터 백업** 버튼 — 상세페이지·썸네일 기록 + 저장된 포맷 + 마진 계산 기록을 JSON 파일로 내보내기/가져오기 (가져올 때 기존 기록에 추가되며 중복은 자동 제거)
...
- 추가 모듈 아이디어(사용자 요청, 2026-09-18): AI CS 상담원 응답 설정 모듈, 관리자 페이지(전체 데이터 대시보드) — 우선순위 미정
