...
- 헤더의 **데이터 백업** 버튼 — 상세페이지·썸네일 기록을 JSON 파일로 내보내기/가져오기 (가져올 때 기존 기록에 추가되며 중복은 자동 제거)
...
- ~~데이터 export/import (JSON)~~ → 완료 (헤더의 "데이터 백업" 버튼, 상세페이지·썸네일 기록을 JSON으로 내보내기/가져오기, 가져올 때 중복 자동 제거)
