...
- 헤더의 **데이터 백업** 버튼 — 상세페이지·썸네일 기록 + 저장된 포맷을 JSON 파일로 내보내기/가져오기 (가져올 때 기존 기록에 추가되며 중복은 자동 제거)
...
- ~~상세페이지 포맷 관리~~ → 완료 (텍스트 구조 템플릿 저장/선택). Figma 연동(실제 시각 디자인 레이아웃)은 다음 단계로 미룸
- 상세페이지 프롬프트 품질 다듬기 (카테고리별 규칙 실사용 테스트 필요)
- 이미지 생성 모델명은 세 회사 모두 자주 바뀌므로, 실제 테스트하면서 최신 모델 ID로 업데이트 필요
- 추가 모듈 아이디어(사용자 요청, 2026-09-18): Figma 연동 디자인 레이아웃, AI CS 상담원 응답 설정 모듈, 마진 계산기, 관리자 페이지(전체 데이터 대시보드) — 우선순위 미정, 하나씩 순서대로 진행 예정
